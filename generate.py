...
data) + values/aar/<site>.yaml (per-site app-aware routing, see
values/aar/_README.md) + globals.yaml (fabric-wide constants) and writes
ordinary NaC YAML into sites/<site>/data/, plus a copy of the Terraform root
files from root-template/. Output under sites/ is GENERATED - never hand-edit it.
...
# Per-site app-aware routing, one file per site: values/aar/<site_code>.yaml.
# A SUBDIRECTORY on purpose — VALUES.glob("*.yaml") below is not recursive, so
# these files are not mistaken for site definitions (they carry none of the
# required site keys and would abort the run).  See values/aar/_README.md.
AAR_VALUES = VALUES / "aar"
...
def generate_site(values_file: pathlib.Path, globals_data: dict, findings: dict) -> str:
    ...
    # Output files are named by BASENAME, so two templates from different
    # directories (e.g. _shared/policy-objects.yaml and branch/policy-objects.yaml)
    # would silently overwrite each other — the later one in the list winning and
    # the earlier one's objects vanishing from the site with no error at all.
    # Bitten once on 2026-09-02 while splitting the policy objects across
    ...
    for rel_path in template_list:
        ...
        if src.name in written:
            raise ValueError(
                f"{site_code}: templates '{written[src.name]}' and '{rel_path}' "
                f"both render to data/{src.name} — the second would silently "
                f"overwrite the first. Give one of them a distinct file name."
            )
        written[src.name] = rel_path
...
def main():
    ...
    # Enforce site_id uniqueness across all active values files (AGENTS.md hard rule).
    seen_ids: dict = {}
    ...
    for vf in all_value_files:
        ...
        if sid in seen_ids:
            sys.exit(
                ...
                f"— site_id must be globally unique (AGENTS.md hard rule)"
            )
